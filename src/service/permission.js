...
class PermissionService {
    constructor() { }
    async addPermission(body, userId) {
        ...
        const result = await pool.query("insert into permissions(user_id,transport_id,can_create,can_read,can_delete,can_update) values($1,$2,$3,$4,$5,$6) returning *", [body.user_id, body.transport_id, body.can_create, body.can_read, body.can_delete, body.can_update])
    ...
    async changePermission(body, userIdToUpdate, user_id) {
        const findAdmin = await pool.query("select * from users where id=$1", [user_id])
        if (findAdmin.rows.length === 0) throw new CustomError("admin not found", 404)
        ...
        const result = await pool.query("update permissions set transport_id=$1,can_create=$2,can_read=$3,can_delete=$4,can_update=$5 where user_id=$6 returning *", [body.transport_id, body.can_create, body.can_read, body.can_delete, body.can_update, userIdToUpdate])
