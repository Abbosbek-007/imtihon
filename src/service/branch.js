...
class BranchService {
    ...
    async addBranch(body, user_id) {
        ...
        const result = await pool.query("insert into branches(name,location) values($1,$2)", [body.name, body.location])
    ...
    async changeBranch(body, branch_id, user_id) {
        const findAdmin = await pool.query("select * from users where id=$1", [user_id])
        if (findAdmin.rows.length === 0) throw new CustomError("admin not found", 404)
        ...
        const result = await pool.query("update branches  name=$1,location=$2 where id=$3 returning * ", [body.name, body.location, branch_id])
