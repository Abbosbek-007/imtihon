...
class TransportService {
    ...
    async addTransport(body, user_id, img) {
        const findAdmin = await pool.query("select * from users where id=$1", [user_id])
        if (findAdmin.rows.length === 0) throw new CustomError("admin not found", 404)
        ...
        const result = await pool.query("insert into transports(model,color,img,price,branch_id) values($1,$2,$3,$4,$5) returning * ", [body.model, body.color, filename, body.price, body.branch_id])
    ...
    async changeTransport(body, transport_id, user_id, img) {
        const findAdmin = await pool.query("select * from users where id=$1", [user_id])
        if (findAdmin.rows.length === 0) throw new CustomError("admin not found", 404)
        ...
        const result = await pool.query("update transports set model=$1,color=$2,img=$3,price=$4,branch_id=$5 where id=$6 returning * ", [body.model, body.color, filename, body.price, body.branch_id, transport_id])
